...
double partition(double x, double y) {
    if(x > 0 && y >= 0)return 0;
    if(x <= 0 && y > 0) return 1;
    if(x < 0 && y <= 0) return 2;
    return 3;
...
bool cmp(const point a, const point b) {
    double ax = a.x - tmp[0].x, ay = a.y - tmp[0].y;
    double bx = b.x - tmp[0].x, by = b.y - tmp[0].y;
    double ap = partition(ax, ay);
    double bp = partition(bx, by);
...
int main() {
    ...
    for(int i = 0; i < n; ++i) {
        ...
        for(int j = i + 1, k = j; j < n; ++j) {
            ...
            if(j == k) continue;
            double vx = sumx[k] - sumx[j];
            double vy = sumy[k] - sumy[j];
            double cnt = k - j;
