...
#define MOT_DC_SHOOT_1  1
#define MOT_DC_SHOOT_2  2

#define SPD_SHOOT     4095
#define SPD_INTAKE    4095
#define SPD_DRIVE     4096
#define SPD_DEAD      80
...
void loop() {
  ...
  ps2.read_gamepad(); // update from controller
   ...
   int16_t speed_l = map(ps2.Analog(PSS_LY), 0, 255, -SPD_DRIVE, SPD_DRIVE); speed_l=speed_l*3/5;
   dc_control(MOT_DC_LEFT, speed_l); Serial.print(speed_l, DEC); Serial.print(' ');
   int16_t speed_r = map(ps2.Analog(PSS_RY), 0, 255, SPD_DRIVE, -SPD_DRIVE);  
   dc_control(MOT_DC_RIGHT, speed_r); Serial.println(speed_r, DEC);
   if (ps2.Button(PSB_RED)){
      dc_control(MOT_DC_SHOOT_1, SPD_SHOOT);
      dc_control(MOT_DC_SHOOT_2, SPD_SHOOT);
   } else {
     dc_control(MOT_DC_SHOOT_1, 0);
     dc_control(MOT_DC_SHOOT_2, 0);
   }
}